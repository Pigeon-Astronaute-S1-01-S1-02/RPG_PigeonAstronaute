...
namespace RPG_PigeonAstronaute.Sprites
{
    ...
    {
        private readonly Ennemie ennemi;
        //Gestion du clavier
        ...
        public Player SetFollowTarget(Player followTarget, float followDistance)
        {
            ennemi.FollowTarget = followTarget;
            ennemi.FollowDistance = followDistance;
            return followTarget;
        }

        protected void Follow()
        {
            if (ennemi.FollowTarget != null)
                return;

            var distance = ennemi.FollowTarget._position - this._position;
            ennemi.Direction = new Vector2((float)Math.Cos(0), (float)Math.Sin(0));
            var currentDir = Vector2.Distance(this._position, ennemi.FollowTarget._position);
            if (currentDir > ennemi.FollowDistance)
            {
                var t = MathHelper.Min((float)Math.Abs(currentDir - ennemi.FollowDistance), this._vitesse);
                var velocity = ennemi.Direction * t;

                ennemi._position += velocity;
